...
class ApplicationController extends AbstractController
{
    ...
    public function docx(string $applicationId): BinaryFileResponse
    {
        ...
        if ($application instanceof EducationalProcess) {
            $adaptations = implode("\n", array_map(
                static fn($adaptation) => '- ' . $adaptation->value,
                $application->adaptations->toArray(),
            ));
        }

        $file = $this->docxGenerator->generate($template, [
            'firstName' => $application->student->firstName,
            'lastName' => $application->student->lastName,
            'albumNumber' => $application->albumNumber,
            'faculty' => $application->faculty,
            'year' => $application->year,
            'semester' => $application->semester,
            'studyMode' => $application->studyMode,
            'adaptationTime' => $application->adaptationCard->value ?? null,
            'adaptations' => $adaptations,
            'academicYear' => '2025/2026',
        ]);

        $resp = new BinaryFileResponse($file);
        $resp->setContentDisposition(ResponseHeaderBag::DISPOSITION_ATTACHMENT, 'kartaAdaptacji.docx');
    ...
    public function pdf(string $applicationId): BinaryFileResponse
    {
        ...
        $resp = new BinaryFileResponse($file);
        $resp->setContentDisposition(ResponseHeaderBag::DISPOSITION_ATTACHMENT, 'kartaAdaptacji.pdf');
        $resp->deleteFileAfterSend(true);
