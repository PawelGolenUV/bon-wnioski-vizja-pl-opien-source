...
class DoctrineAuditSubcriber implements EventSubscriber
{
    ...
    public function onFlush(OnFlushEventArgs $args): void
    {
        ...
        foreach ($uow->getScheduledEntityInsertions() as $entity) {
            ...
            $em->persist($log);
            // Nie wolno zapominać: zarejestrować zmianę encji logu
        ...
        foreach ($uow->getScheduledEntityUpdates() as $entity) {
            ...
            // Pomijamy puste zmiany (po maskowaniu)
            if ($norm === []) {
                continue;
            }
    ...
    {
        // Uproszczona serializacja wartości do JSON
        if (is_object($value)) {
            if ($value instanceof \DateTimeInterface) {
                return $value->format(\DateTimeInterface::ATOM);
            }
            // Relacje — logujemy tylko klasę + id jeżeli dostępne
